...
const CARD_TYPE_PATTERNS_JA = [
  ...
  /^HP\d*$/,
  // ポケモンカードのトレーナーズカード種別行を除外する（例: "サポート"、"グッズ"、"スタジアム"）
  // これらはカード名ではなくカード種別を示す行であり、MTGには同名カードが存在しないため安全に除外できる
  /^(サポート|グッズ|スタジアム|トレーナーズ|ポケモンのどうぐ)$/,
  // 中黒（・ U+30FB）で始まる行はOCRノイズ行であり、カード名ではない
  // 例: "グッズ" が "・ズ" と誤読された場合など、先頭文字が記号に化けたトレーナーズ種別行が該当する
  // MTG・ポケモンともにカード名が中黒で始まることはないため安全に除外できる
  /^・/,
];
...
function extractCardName(lines: string[]): string | null {
  for (const line of lines) {
    ...
    // OCRノイズで末尾に " x 210円"、" × 150ダメージ"、" ¥2100"、" 210円" のようなダメージ・価格行が混入した場合を除去する
    // カード名行に別行のダメージ量テキストや価格テキストが結合されるケースで発生するため、" (¥/x/×) 数字" 以降を削除する
    // "¥/￥" (U+00A5/U+FFE5) はOCRが価格テキスト "¥2100" を同じ行に混入させた場合に前置される
    // "x/×" はオプション（?）にすることで、乗算記号なしで直接 " 210円" と付くケース（ワザダメージ）も除去できる
    // カード名に "円" や "点" が含まれるMTG/ポケモンカードは存在しないため、これらを含む末尾サフィックスは安全に除去できる
    // また、OCRが "× 210円" や "140ダメージ" を "2100" や "140" と数字のみに誤読するケースがある
    // 日本語カード名が数字のみで終わることはないため、スペース+2桁以上の数字のみで終わる末尾も安全に除去できる
    const trailingNoiseMatch = /\s+(?:[¥￥x×]\s*)?\d{2,}[円点ダ]?.*$/.exec(cleanedLine);
